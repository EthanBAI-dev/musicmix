...
.. warning::
   **实测结论（2026-08-01，MUSDB18-HQ test 50 首）：掩码类细化在 htdemucs 上是净损失，别再试了。**

   ==================  ==========  ==============
   手段                 平均 Δ      结论
   ==================  ==========  ==============
   TTA(swap, flip)     **+0.095**  四轨全部显著变好，但代价 ×2.9 推理时间
   overlap 0.25→0.50   +0.037      整体不显著
   ``soft_mask_refine``  **-1.357**  显著变差
   ``multichannel_wiener``  **-0.923**  显著变差
   ==================  ==========  ==============

   跑之前我在这里写过一条预测：既然 P1 量到 IRM oracle 比 htdemucs 在 other 上高 2.59 dB，
   掩码细化就该在 other/vocals 上有增益。**四条全错**，见 ``results/P2_分析.md``。

   **机制已由隔离实验证实**（``scripts/diagnose_mask_loss.py``，10 首）：
   把"换相位"和"重分配幅度"拆开单独测，**相位那一步占总损失的 102%**
   （只换相位 -1.09 dB，幅度重分配 +0.02 dB）。
   也就是说损失几乎全部来自**掩码必然继承混音的相位**，
   而 Demucs 直接生成波形、能重建相位。bass/drums 受损最重（-1.91 / -1.62 dB），
   正因为低频与瞬态最依赖相位。

   错因：**那 2.59 dB 的头寸只有拿到真值幅度才够得着。**
   掩码细化的本质是把估计投影到"混音复数谱的实值缩放"这个集合上，
   而该集合的最优点（IRM oracle 8.95 dB）距离 htdemucs 的 base（8.80 dB）**只有 0.15 dB**。
   投影到一个天花板只比你高 0.15 dB 的集合里，不可能拿到 2.59 dB。

   两个函数保留下来是因为它们在**弱基线**上仍然有用
   （单测里对含噪估计确实能提升），而且是 IRM oracle 的实现参照。
   **但不要再拿它们去"改进" htdemucs 级别的模型。**
"""
